package work.licht.music.user.runner;
...
import work.licht.music.user.domain.mapper.PermissionDOMapper;
import work.licht.music.user.domain.mapper.RoleDOMapper;
import work.licht.music.user.domain.mapper.RolePermissionDOMapper;
...
public class PushRolePermissions2RedisRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        ...
        try {
            // 是否能够同步数据
            // 原子操作，只有在键 PUSH_PERMISSION_FLAG 不存在时，才会设置该键的值为 "1"，并设置过期时间为 1 天
            ...
            if (CollUtil.isNotEmpty(roles)) {
                ...
                roleKeyPermissionsMap.forEach((roleKey, strings) -> {
                    String key = RedisKeyConstants.buildRolePermissionsKey(roleKey);
                    redisTemplate.opsForValue().set(key, gson.toJson(strings));
                });
            }
