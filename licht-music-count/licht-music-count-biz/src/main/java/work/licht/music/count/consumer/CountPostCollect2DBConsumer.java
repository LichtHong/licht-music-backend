...
@RocketMQMessageListener(
        ...
        topic = RocketMQConstants.TOPIC_COUNT_POST_COLLECT_2_DB // 主题 Topic
)
@Slf4j
public class CountPostCollect2DBConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String body) {
        ...
        rateLimiter.acquire();
        log.info("## 消费到了 MQ 【计数: 帖子收藏数入库】, {}...", body);
        List<AggregationCountCollectUncollectPostMqDTO> countList = null;
        try {
        ...
        if (CollUtil.isNotEmpty(countList)) {
            ...
            countList.forEach(item -> {
                ...
                transactionTemplate.execute(status -> {
                    try {
                        postCountDOMapper.insertOrUpdateCollectTotalByPostId(count, postId);
