...
import work.licht.music.count.enums.FollowUnfollowTypeEnum;
import work.licht.music.count.model.dto.CountFollowUnfollowMqDTO;
...
@RocketMQMessageListener(
        ...
        topic = RocketMQConstants.TOPIC_COUNT_FOLLOWER // 主题 Topic
)
@Slf4j
public class CountFollowerConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> messages) {
        ...
        log.info("==> 聚合消息, {}", gson.toJson(messages));
        // List<String> 转 List<CountFollowUnfollowMqDTO>
        List<CountFollowUnfollowMqDTO> countFollowUnfollowMqDTOS = messages.stream().map(body ->
            gson.fromJson(body, CountFollowUnfollowMqDTO.class)
        ).toList();
        // 按目标用户进行分组
        ...
        log.info("## 聚合后的计数数据: {}", gson.toJson(countMap));
        // 更新 Redis
        countMap.forEach((k, v) -> {
            ...
            // 若存在才会更新 (缓存设有过期时间，过期后，缓存会被删除，这里需要判断，存在才会去更新，而初始化工作放在查询计数来做)
            if (isExisted) {
                // 对目标用户 Hash 中的粉丝数字段进行计数操作
                redisTemplate.opsForHash().increment(redisKey, RedisKeyConstants.FIELD_FOLLOWER_TOTAL, v);
            }
        });
        // 发送 MQ, 计数数据落库
        // 构建消息体 DTO
        Message<String> message = MessageBuilder.withPayload(gson.toJson(countMap)).build();
        // 异步发送 MQ 消息，提升接口响应速度
        rocketMQTemplate.asyncSend(RocketMQConstants.TOPIC_COUNT_FOLLOWER_2_DB, message, new SendCallback() {
            @Override
