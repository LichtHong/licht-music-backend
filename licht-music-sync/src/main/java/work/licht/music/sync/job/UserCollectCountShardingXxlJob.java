...
public class UserCollectCountShardingXxlJob {
    ...
    public void userCollectCountShardingJobHandler() {
        ...
        for (;;) {
            ...
            List<Long> userIds = selectMapper.selectBatchFromSyncUserCollectCountTempTable(tableNameSuffix, batchSize);
            // 若记录为空，终止循环
            if (CollUtil.isEmpty(userIds)) break;
            // 循环这一批发生变更的用户 ID
            userIds.forEach(userId -> {
                // 2: 对 t_user_collection 用户收藏表执行 count(*) 操作，获取用户获得的收藏总数
                int userCollectTotal = selectMapper.selectUserCollectCountFromPostCollectionTableByUserId(userId);
            ...
            // 4. 批量物理删除这一批次记录
            deleteMapper.batchDeleteSyncUserCollectCountTempTable(tableNameSuffix, userIds);
            // 当前已处理的记录数
            processedTotal += userIds.size();
        }
        XxlJobHelper.log("=================> 结束定时分片广播任务：对当日发生变更用户获得的收藏数进行同步，共同步记录数：{}", processedTotal);
    }
