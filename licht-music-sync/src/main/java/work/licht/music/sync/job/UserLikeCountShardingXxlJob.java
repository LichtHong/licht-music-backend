...
public class UserLikeCountShardingXxlJob {
    ...
    public void userLikeCountShardingJobHandler() {
        ...
        XxlJobHelper.log("=================> 开始定时分片广播任务：对当日发生变更用户获得的点赞数进行同步");
        ...
        for (;;) {
            ...
            List<Long> userIds = selectMapper.selectBatchFromSyncUserLikeCountTempTable(tableNameSuffix, batchSize);
            // 若记录为空，终止循环
            if (CollUtil.isEmpty(userIds)) break;
            // 循环这一批发生变更的用户 ID
            userIds.forEach(userId -> {
                // 2: 对 t_user_like 用户点赞表执行 count(*) 操作，获取用户获得的点赞总数
                int userLikeTotal = selectMapper.selectUserLikeCountFromPostLikeTableByUserId(userId);
                // 3: 更新 t_user_count 表
            ...
            // 4. 批量物理删除这一批次记录
            deleteMapper.batchDeleteSyncUserLikeCountTempTable(tableNameSuffix, userIds);
            // 当前已处理的记录数
            processedTotal += userIds.size();
        }
        XxlJobHelper.log("=================> 结束定时分片广播任务：对当日发生变更用户获得的点赞数进行同步，共同步记录数：{}", processedTotal);
    }
