package work.licht.music.id.core.snowflake;
...
import work.licht.music.id.core.snowflake.exception.CheckLastTimeException;
...
public class SnowflakeZookeeperHolder {
        ...
        try {
            ...
            if (stat == null) {
                ...
                if (workerId != null) {
                    ...
                    doService(curator);
                    updateLocalWorkerID(workerID);
                    LOGGER.info("[Old NODE]find forever node have this endpoint ip-{} port-{} workid-{} child node and start SUCCESS", ip, port, workerID);
                } else {
            ...
            LOGGER.error("Start node ERROR {}", e.toString());
            try {
    ...
    private void updateNewData(CuratorFramework curator, String path) {
        try {
            ...
            LOGGER.info("update init data error path is {} error is {}", path, e.toString());
        }
    }
