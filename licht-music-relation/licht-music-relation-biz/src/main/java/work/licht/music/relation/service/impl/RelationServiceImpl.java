...
public class RelationServiceImpl implements RelationService {
    ...
    // 构建 Lua 脚本参数
    private static Object[] buildLuaArgs(List<FollowingDO> followingDOList, long expireSeconds) {
    ...
    public Response<?> unfollow(UnfollowUserReqVO unfollowUserReqVO) {
        ...
        if (Objects.equals(result, LuaResultEnum.ZSET_NOT_EXIST.getCode())) { // ZSET 关注列表不存在
            ...
            if (CollUtil.isEmpty(followingDOList)) throw new BizException(ResponseCodeEnum.NOT_FOLLOWED);
            // 若记录不为空，则将关注关系数据全量同步到 Redis 中，并设置过期时间；
            // 执行 Lua 脚本，批量同步关注关系数据到 Redis 中
            DefaultRedisScript<Long> script3 = new DefaultRedisScript<>();
            script3.setScriptSource(new ResourceScriptSource(new ClassPathResource("/lua/follow_batch_add_and_expire.lua")));
    ...
    public PageResponse<FindFollowingUserRespVO> findFollowingList(FindFollowingListReqVO findFollowingListReqVO) {
        ...
        if (total > 0) { // 缓存中有数据
            ...
            if (CollUtil.isNotEmpty(followingUserIdSet)) {
                // 提取所有用户 ID 到集合中
                List<Long> userIdList = followingUserIdSet.stream().map(object -> ((Number) object).longValue()).toList();
                // RPC: 批量查询用户信息
                findFollowingUserRespVOS = rpcUserServiceAndDTO2FollowingVO(userIdList);
            }
        } else {
            ...
            // 分页查询
            List<FollowingDO> followingDOList = followingDOMapper.selectPageListByUserId(userId, offset, limit);
            // 若记录不为空
            if (CollUtil.isNotEmpty(followingDOList)) {
                // 提取所有关注用户 ID 到集合中
                List<Long> userIdList = followingDOList.stream().map(FollowingDO::getFollowingUserId).toList();
    ...
    private List<FindFollowingUserRespVO> rpcUserServiceAndDTO2FollowingVO(List<Long> userIdList) {
        // RPC: 批量查询用户信息
    ...
    public PageResponse<FindFollowerUserRespVO> findFollowerList(FindFollowerListReqVO findFollowerListReqVO) {
        ...
        if (total > 0) { // 缓存中有数据
            ...
            if (CollUtil.isNotEmpty(followingUserIdsSet)) {
                ...
                // RPC: 批量查询用户信息
                findFollowerUserRespVOS = rpcUserServiceAndDTO2FollowerVO(userIds);
            }
        } else { // 若 Redis 缓存中无数据，则查询数据库
            // 先查询记录总量
            total = followerDOMapper.selectCountByUserId(userId);
            // 计算一共多少页
            long totalPage = PageResponse.getTotalPage(total, limit);
            ...
            if (CollUtil.isNotEmpty(followerDOList)) {
                // 提取所有粉丝用户 ID 到集合中
                List<Long> userIds = followerDOList.stream().map(FollowerDO::getFollowerUserId).toList();
    ...
    private void syncFollowerList2Redis(Long userId) {
        List<FollowerDO> followerDOList = followerDOMapper.selectListByUserId(userId, 5000);
        if (CollUtil.isNotEmpty(followerDOList)) {
            // 用户粉丝列表 Redis Key
            String followerListRedisKey = RedisKeyConstants.buildUserFollowerKey(userId);
            // 随机过期时间
            ...
            long expireSeconds = 60*60*24 + RandomUtil.randomInt(60*60*24);
            // 构建 Lua 参数
            Object[] luaArgs = buildFollowerZSetLuaArgs(followerDOList, expireSeconds);
            // 执行 Lua 脚本，批量同步关注关系数据到 Redis 中
            DefaultRedisScript<Long> script = new DefaultRedisScript<>();
            script.setScriptSource(new ResourceScriptSource(new ClassPathResource("/lua/follow_batch_add_and_expire.lua")));
            script.setResultType(Long.class);
    ...
    private static Object[] buildFollowerZSetLuaArgs(List<FollowerDO> followerDOList, long expireSeconds) {
